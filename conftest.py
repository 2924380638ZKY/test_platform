...
@pytest.hookimpl(hookwrapper=True)  # 钩子函数，获取测试用例不同阶段的测试结果
def pytest_runtest_makereport(item):  # 当测试失败的时候，自动截图，展示到html报告中
    pytest_html = item.config.pluginmanager.getplugin('html')
    outcome = yield
    report = outcome.get_result()
    report.description = str(item.function.__doc__)
    extra = getattr(report, 'extra', [])  # 返回report对象属性extra值
    if report.when == 'call':
        xfail = hasattr(report, 'wasxfail')  # 判断是否有该属性 ，xfail是预期错误标志
...
def _capture_screenshot_fail(item):  # 截图保存为base64
    screen_file = Util.screen_path_fail(Util, item)
    print(driver)
...
def _capture_screenshot_succ(item):  # 截图保存为base64
    screen_file = Util.screen_path_succ(Util, item)
    print(driver)
    driver.save_screenshot(screen_file)
